...
try {
    $pdo =new PDO("mysql:host=localhost;dbname=11/01",'root','');
...
if(!empty($_POST) && !empty($_FILES)){
  ...
  // constante : Taille max en octets du fichie
    ...
    $firstName=strip_tags(trim($_POST['nom']));
    $lastName=strip_tags(trim($_POST['prenom']));
    $mailAdresse=strip_tags(trim($_POST['email']));
        ...
        if(preg_match("/^[a-zA-Z-']*$/",$firstName)){
            if(preg_match("/^[a-zA-Z-']*$/",$lastName)){
                if (preg_match("/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/",$mailAdresse)) {
                    ...
                    $request=$pdo->prepare("INSERT INTO essaie(nom,prenom,email,mdp,photo) VALUES (?,?,?,?,?)");
                    $request->execute([$firstName, $lastName, $mailAdresse, $mdp,$file_dest]); 
